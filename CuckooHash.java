...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        for (int k = 0; k < CAPACITY; k++) {
            if (table[k] != null && table[k].getBucKey().equals(key) && table[k].getValue().equals(value))
                return;
        }
        ...
        while (count < cap) {
            ...
            Bucket<K, V> displaced = table[position];
            table[position] = newEntry;

            K displacedKey = displaced.getBucKey();
            int p1 = hash1(displacedKey);
            int p2 = hash2(displacedKey);
